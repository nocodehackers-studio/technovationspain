...
import { Scale, Mail, ArrowLeft, Loader2, Info, KeyRound } from 'lucide-react';
...
export default function RegisterJudge() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${baseUrl}/auth/callback?role=judge`,
        data: {
          intended_role: 'judge'
        }
      },
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (data.session) {
        ...
        navigate('/onboarding?role=judge');
      }
    } catch (error: any) {
        ...
        <Card className="w-full max-w-md card-hover">
          ...
          <CardContent className="space-y-6">
            <p className="text-center text-sm text-muted-foreground">
              Haz clic en el enlace del correo para completar tu registro como juez. 
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-secondary via-background to-muted p-4">
      <Card className="w-full max-w-md card-hover">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-xl font-display">Registro de Juez</CardTitle>
          <CardDescription className="text-base">
            Para profesionales mayores de 18 años que evalúan proyectos
          </CardDescription>
        </CardHeader>
          ...
          <CardContent className="space-y-4">
            <Alert className="border-muted bg-accent/5">
              ...
              <AlertDescription className="text-sm">
                <strong>Requisito:</strong> Debes tener al menos 18 años. Los jueces evalúan 
                los proyectos de las estudiantes en eventos regionales y nacionales.
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Usa un email profesional o personal donde podamos contactarte
              </p>
            </div>
