...
import { Heart, Mail, ArrowLeft, Loader2, Info, KeyRound, Home } from 'lucide-react';
...
export default function RegisterVolunteer() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${baseUrl}/auth/callback?role=volunteer`,
        data: {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (data.session) {
        ...
        navigate('/onboarding?role=volunteer');
      }
    } catch (error: any) {
  ...
  if (emailSent) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-accent/20 via-background to-muted p-4">
        <Card className="w-full max-w-md card-hover">
          ...
          <CardContent className="space-y-6">
            <p className="text-center text-sm text-muted-foreground">
              Haz clic en el enlace del correo para completar tu registro como voluntario/a. 
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-accent/20 via-background to-muted p-4">
      <Card className="w-full max-w-md card-hover">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-xl font-display">Registro de Voluntario/a</CardTitle>
          <CardDescription className="text-base">
            Únete a nuestro equipo para apoyar eventos de Technovation
          </CardDescription>
        </CardHeader>
          ...
          <CardContent className="space-y-4">
            <Alert className="border-muted bg-accent/5">
              ...
              <AlertDescription className="text-sm">
                <strong>Como voluntario/a</strong> podrás apuntarte a eventos para ayudar con la 
                logística, validación de entradas y apoyo general. Tu perfil se verificará 
                automáticamente.
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Usaremos este email para contactarte sobre los eventos
              </p>
            </div>
